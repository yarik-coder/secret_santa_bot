...
def komnata(message):
    global wait, wait1, n, name, wait2, wait_for_start, proverkaa, wait_comment, a, cont, wait_comment_admin, comment, result_comment, wait_admin_panel, show_people, delete_people, delete_people2, komnata_for_de
    result_comment = ""
        ...
        for i in komnaty_p_n_a_p:
            if i == proverka_prav:
                ...
                if message.from_user.id == int(proverka_p_n_a_p.readline().split()[1]):
                    ...
                    btn5 = types.KeyboardButton("закрыть комнату")
                    btn6 = types.KeyboardButton("выйти из панели админа")
