        ...
        for i in komnaty:
            ...
            if i == name_proverki:
                ...
                if message.from_user.id == int(proverka.readline().split()[1]):
                    ...
                    proverka = open(name_proverki, "r")
                    lines = proverka.readlines()
                    gifts = open(name_proverki, "a")
                    linesss = ""
                    for line in lines:
                        linesss = linesss + line.split()[0] + " "
                    print(linesss.split()[0] + " " + linesss.split()[peremeshanie])
                    linessss = ""
                    for line in lines:
                        linessss = linessss + str(line.split()[1] + " ")
                    print(linessss.split()[0] + " " + linesss.split()[peremeshanie])
                    pon = 0
                    for line in lines:
                        id = linessss.split()[pon]
                        if pon == peremeshanie:
                            peremeshanie = 0
                        bot.send_message(message.from_user.id, "ты даришь подарок " + linesss.split()[peremeshanie])
                        print(linessss.split()[pon] + " ты даришь подарок " + linesss.split()[peremeshanie])
                        pon += 1
