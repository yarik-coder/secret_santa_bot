...
def komnata(message):
    global wait, wait1, n, name, wait2, wait_for_start, proverkaa, wait_comment, a, cont, wait_comment_admin, comment, result_comment, wait_admin_panel, show_people, delete_people, delete_people2, komnata_for_de, switch_name, switch_name2, komnata_switch_name, proverka_prav, old_name
    result_comment = ""
    if message.text == "создать комнату":
        ...
        '''
        if proverka_prav == None:
            bot.send_message(message.from_user.id, "вы не ввели название комнаты")
        elif proverka_prav.split()[0] == "komnata":
            komnata = proverka_prav
            ...
            komnata = "komnata " + proverka_prav + ".txt"
            people = open(komnata, "r", encoding="UTF-8")
            lines = people.readlines()
            id_users = []
            us_users = []
        ...
        if proverka_prav == None:
            ...
            old_name = "komnata " + message.text + ".txt"
            bot.send_message(message.from_user.id, "введи новое имя для комнаты")
            switch_name = True
        ...
        bot.send_message(message.from_user.id, f"комната {old_name} была переименована в {new_name}")
        switch_name = False
        ...
        bot.send_message(message.from_user.id, "такой комнады не существует")
